...
class CrimeSpider:
    ...
    def get_html(self, url,encoding='UTF-8'):
        ...
        html = res.read().decode(encoding='gbk', errors='ignore')
        return html

    '''url解析'''
    def url_parser(self, content,encoding='UTF-8'):
        selector = etree.HTML(content)
        urls = ['http://www.anliguan.com' + i for i in  selector.xpath('//h2[@class="item-title"]/a/@href')]
        return urls

    '''测试'''
    def spider_main(self,encoding='UTF-8'):
    ...
    '''基本信息解析'''
    def basicinfo_spider(self, url,encoding='UTF-8'):
    ...
    '''treat_infobox治疗解析'''
    def treat_spider(self, url,encoding='UTF-8'):
        html = self.get_html(url)
        selector = etree.HTML(html)
    ...
    '''treat_infobox治疗解析'''
    def drug_spider(self, url,encoding='UTF-8'):
        html = self.get_html(url)
        selector = etree.HTML(html)
    ...
    '''food治疗解析'''
    def food_spider(self, url,encoding='UTF-8'):
        html = self.get_html(url)
        selector = etree.HTML(html)
    ...
    '''症状信息解析'''
    def symptom_spider(self, url,encoding='UTF-8'):
        html = self.get_html(url)
        selector = etree.HTML(html)
    ...
    '''检查信息解析'''
    def inspect_spider(self, url,encoding='UTF-8'):
        html = self.get_html(url)
        selector = etree.HTML(html)
    ...
    '''通用解析模块'''
    def common_spider(self, url,encoding='UTF-8'):
        html = self.get_html(url)
        selector = etree.HTML(html)
        ps = selector.xpath('//p')
        infobox = []
        ...
        return '\n'.join(infobox)
    '''检查项抓取模块'''
    def inspect_crawl(self,encoding='UTF-8'):
